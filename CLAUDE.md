...
This file applies to the whole repository. `apps/mobile/CLAUDE.md` /
`apps/mobile/AGENTS.md` add Expo/React-Native-specific rules on top of this —
read both before touching code in `apps/mobile/`.
...
This is an npm-workspaces monorepo (`apps/*`), set up ahead of the
business/staff/admin web dashboard. There is still only one app —
`apps/mobile`. Don't add `packages/` or a second app under `apps/` without
being asked; when the web dashboard work starts, add it as `apps/web` and
pull out shared code (types, Supabase client) into `packages/` at that point.
...
4. **Categories are data, not code.** Don't hard-code category lists in
   components; read from `CATEGORIES` (`apps/mobile/src/lib/placeholder-data.ts`
   today, the real `categories` table after Phase 2) and keep
   [`docs/03-categories.md`](./docs/03-categories.md) in sync.
5. **Placeholder vs. real data.** Code in
   `apps/mobile/src/lib/placeholder-data.ts` is temporary. When wiring a
   screen to real Supabase data, remove the placeholder import for that
   screen rather than leaving both in place.
6. **Naming:** files in `apps/mobile/src/app/` are routes (expo-router) —
   keep non-route code in `components/`, `hooks/`, `lib/`, `store/`,
   `types/`. Use kebab-case for filenames, PascalCase for
   components/types, camelCase for variables/functions, matching the
   existing files.
...
From `apps/mobile/` (or `npm run typecheck` / `npm run lint` from the repo
root): `npx tsc --noEmit` and `npx expo lint`. For Supabase changes:
